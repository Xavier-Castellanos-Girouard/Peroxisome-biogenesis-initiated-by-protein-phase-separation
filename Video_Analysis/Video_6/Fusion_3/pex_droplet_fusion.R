...
aspect_ratio <- data.frame(values = as.numeric(aspect_ratio_raw[1,310:370]))
...
aspect_ratio$major_axis = as.numeric(major_axis_raw[1,310:370])
...
# Plot decay functio
plot(eDecay(t = (analysis_slice_df$time), IniAR = initial_aspect_ratio, 0.06101))
...
fit_model1 <- 
  nls(values ~ eDecay(time, initial_aspect_ratio, tau), 
      ...
      start = list(tau = 0.06101)) # 0.06101

print(fit_model1) # 0.06101
