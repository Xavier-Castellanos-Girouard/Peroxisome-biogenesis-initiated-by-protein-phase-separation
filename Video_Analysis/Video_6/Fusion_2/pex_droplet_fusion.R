...
aspect_ratio <- data.frame(values = as.numeric(aspect_ratio_raw[1,20:90]))
...
aspect_ratio$major_axis = as.numeric(major_axis_raw[1,20:90])
...
# Plot decay functio
plot(eDecay(t = (analysis_slice_df$time), IniAR = initial_aspect_ratio, 0.1285))
...
fit_model1 <- 
  nls(values ~ eDecay(time, initial_aspect_ratio, tau), 
      ...
      start = list(tau = 0.1285)) # 0.1285

print(fit_model1) # 0.1285
