...
# Import values
aspect_ratio_raw <- read.csv(file = "aspect_ratio_values_faster.csv", header = FALSE)
major_axis_raw <- read.csv(file = "axis_major_values_faster.csv", header = FALSE)


# Make dataframe from values in pertinent range
aspect_ratio <- data.frame(values = as.numeric(aspect_ratio_raw[1,240:300]))
...
aspect_ratio$major_axis = values = as.numeric(major_axis_raw[1,240:300])
...
time_slice_start = which(aspect_ratio$time == 0.54)
time_slice_end = which(aspect_ratio$time == 1.74)
...
# Plot decay functio
plot(eDecay(t = (analysis_slice_df$time), IniAR = initial_aspect_ratio, 0.2211))
