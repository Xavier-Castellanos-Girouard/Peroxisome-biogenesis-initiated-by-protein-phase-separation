...
aspect_ratio <- data.frame(values = as.numeric(aspect_ratio_raw[1,230:290]))
...
aspect_ratio$major_axis = values = as.numeric(major_axis_raw[1,230:290])
...
time_slice_start = 18
#which(aspect_ratio$time == 0.60)
time_slice_end = 61
#which(aspect_ratio$time == 1.23)
...
# Plot decay functio
plot(eDecay(t = (analysis_slice_df$time), IniAR = initial_aspect_ratio, 0.04425))
...
fit_model1 <- 
  nls(values ~ eDecay(time, initial_aspect_ratio, tau), 
      ...
      start = list(tau = 0.04425)) # 0.04425

print(fit_model1) # 0.04425
