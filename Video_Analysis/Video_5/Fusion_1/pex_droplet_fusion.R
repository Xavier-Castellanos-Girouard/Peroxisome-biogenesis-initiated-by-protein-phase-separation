...
aspect_ratio <- data.frame(values = as.numeric(aspect_ratio_raw[1,410:470]))
...
aspect_ratio$major_axis = as.numeric(major_axis_raw[1,410:470])
...
# Plot decay functio
plot(eDecay(t = (analysis_slice_df$time), IniAR = initial_aspect_ratio, 0.09379))
...
fit_model1 <- 
  nls(values ~ eDecay(time, initial_aspect_ratio, tau), 
      ...
      start = list(tau = 0.09379)) # 0.09379

print(fit_model1) # 0.09379
