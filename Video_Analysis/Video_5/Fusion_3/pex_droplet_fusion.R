...
aspect_ratio <- data.frame(values = as.numeric(aspect_ratio_raw[1,450:500]))
...
aspect_ratio$major_axis = as.numeric(major_axis_raw[1,450:500])
...
# Plot decay functio
plot(eDecay(t = (analysis_slice_df$time), IniAR = initial_aspect_ratio, 0.06986))
...
fit_model1 <- 
  nls(values ~ eDecay(time, initial_aspect_ratio, tau), 
      ...
      start = list(tau = 0.06986)) # 0.06986

print(fit_model1) # 0.06986
...
pdf("Figure5C_panel2_pub.pdf")
plot2
dev.off()


tiff("pub.tiff", units="in", width=5, height=5, res=1200)
plot2
dev.off()
