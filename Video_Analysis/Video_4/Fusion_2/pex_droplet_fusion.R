...
aspect_ratio <- data.frame(values = as.numeric(aspect_ratio_raw[1,420:500]))
...
aspect_ratio$major_axis = as.numeric(major_axis_raw[1,420:500])
...
# Plot decay functio
plot(eDecay(t = (analysis_slice_df$time), IniAR = initial_aspect_ratio, 0.123))
...
fit_model1 <- 
  nls(values ~ eDecay(time, initial_aspect_ratio, tau), 
      ...
      start = list(tau = 0.123)) # 0.123

print(fit_model1) # 0.123
