...
aspect_ratio <- data.frame(values = as.numeric(aspect_ratio_raw[1,200:250]))
...
aspect_ratio$major_axis = as.numeric(major_axis_raw[1,200:250])
...
# Plot decay functio
plot(eDecay(t = (analysis_slice_df$time), IniAR = initial_aspect_ratio, 0.08724))
...
fit_model1 <- 
  nls(values ~ eDecay(time, initial_aspect_ratio, tau), 
      ...
      start = list(tau = 0.08724)) # 0.08724

print(fit_model1) # 0.08724
