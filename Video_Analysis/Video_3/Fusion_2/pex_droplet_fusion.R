...
# Import values
aspect_ratio_raw <- read.csv(file = "aspect_ratio_values.csv", header = TRUE)
major_axis_raw <- read.csv(file = "axis_major_values.csv", header = TRUE)


# Make dataframe from values in pertinent range
aspect_ratio <- data.frame(values = as.numeric(aspect_ratio_raw[1,65:110]))
...
aspect_ratio$major_axis = as.numeric(major_axis_raw[1,65:110])
...
time_slice_start = 17
#which(aspect_ratio$time == 0.66)
time_slice_end = 43
#which(aspect_ratio$time == 1.32)
...
# Plot decay functio
plot(eDecay(t = (analysis_slice_df$time), IniAR = initial_aspect_ratio, 0.1126))



# Make model to find fit tau
fit_model1 <- 
  nls(values ~ eDecay(time, initial_aspect_ratio, tau), 
      data = analysis_slice_df, 
      start = list(tau = 0.1126)) # 0.1126

print(fit_model1) # 0.1126
